// public/js/relics.js
window.relics = [
    {
        id: 1,
        lotNumber: '001',
        title: 'AN EXCEPTIONALLY RARE RU KILN SUNFLOWER-FORM WASHER',
        chinese: '北宋 汝官窯天青釉葵花洗',
        dynasty: 'song',
        dynastyInfo: 'Northern Song Dynasty, 1086-1106',
        dynastyInfoChinese: '北宋 (1086-1106年)',
        imageOrientation: 'landscape',
        images: [
            '/assets/images/relics/001-song-ru-washer/main.jpg',
            '/assets/images/relics/001-song-ru-washer/detail-1.jpg',
            '/assets/images/relics/001-song-ru-washer/detail-2.jpg',
            '/assets/images/relics/001-song-ru-washer/detail-3.jpg',
        ],
        thumbnail: '/assets/images/relics/001-song-ru-washer/main.jpg',
        description: 'This extraordinarily rare washer represents the pinnacle of Song ceramic achievement. The vessel is shaped like a six-part flower mouth with three tiny sesame nail marks on the bottom. The distinctive sky-blue glaze with its subtle crackle pattern was considered the finest of all Chinese ceramics. The azure glaze is particularly notable for its purity and the intricate network of ice cracks, which are a hallmark of Ru ware.',
        descriptionChinese: '這件極其稀有的洗器代表了宋代陶瓷工藝的巔峰。器物呈六瓣花口狀，底部有三個細小的芝麻釘痕。其獨特的天青釉和細密的開片紋被認為是所有中國陶瓷中最精美的。天青釉以其純淨和冰裂紋的複雜網絡而聞名，這是汝窯的標誌性特徵。',
        estimate: { low: 40000000, high: 45000000 }, // USDT
        currentBid: 40000000, // USDT
        provenance: 'Private Collection, Hong Kong\nFormed in the 1970s\nAcquired from J.J. Lally & Co., New York, 1985\nJ.J. Lally & Co. is a renowned dealer of Chinese art and ceramics based in New York',
        provenanceChinese: '私人收藏，香港\n形成於1970年代\n1985年購自紐約J.J. Lally & Co.\nJ.J. Lally & Co. 是紐約著名的中國藝術和陶瓷經銷商',
        condition: 'Excellent condition with characteristic ice-crackle throughout the glaze. The outer wall features diagonal cuts with willow silk and ice cracks, while the inside is also adorned with ice cracks. Minor expected wear to the foot ring consistent with age.',
        conditionChinese: '保存完好，釉面遍布特徵性的冰裂紋。外壁有柳葉紋和冰裂紋的斜切痕跡，內壁也裝飾有冰裂紋。足圈有與年代相符的輕微磨損。',
        dimensions: 'Diameter: 14.6 cm\nBottom diameter: 9.6 cm\nHeight: 3.65 cm\nWeight: 285g',
        dimensionsChinese: '直徑：14.6厘米\n底徑：9.6厘米\n高：3.65厘米\n重量：285克',
        literature: 'Compare a similar example in the Palace Museum, Beijing, illustrated in "Complete Collection of Treasures of the Palace Museum: Porcelain of the Song Dynasty (I)", Hong Kong, 1996, pl. 5. A Ru ware brush washer sold for HK$294.3 million (US$37.7 million) at Sotheby\'s Hong Kong on October 3, 2017, setting a world record for Chinese ceramics.',
        literatureChinese: '可比較北京故宮博物院的一件類似藏品，見《故宮博物院藏品大系：宋代陶瓷卷（一）》，香港，1996年，圖5。一件汝窯洗器於2017年10月3日在蘇富比香港拍賣會上以2.943億港元（3770萬美元）成交，創下中國陶瓷的世界紀錄。',
        technicalNotes: 'The glaze exhibits the characteristic "sky-blue" color achieved through reduction firing. The crackled glaze effect, known as "crazing," was highly prized and added to the visual and tactile appeal of the piece. X-ray fluorescence analysis could confirm the presence of agate powder in the glaze composition, a hallmark of authentic Ru ware.',
        technicalNotesChinese: '釉面呈現通過還原燒製實現的特徵性"天青色"。開片釉效果，即"蟹爪紋"，極受推崇，增加了器物的視覺和觸覺美感。X射線熒光分析可以確認釉料成分中含有瑪瑙粉，這是真品汝窯的標誌。',
        culturalSignificance: 'Ru kilns operated for only about 20 years (1086-1106) exclusively for the Northern Song court. Fewer than 100 genuine pieces are known to exist worldwide, with the Taipei National Palace Museum holding 21 pieces and the Beijing Palace Museum having 17 pieces. A common saying goes: "Even with a fortune, a single piece of Ru porcelain is priceless" (纵有家财万贯，不抵汝瓷一件).',
        culturalSignificanceChinese: '汝窯僅運營了約20年（1086-1106年），專門為北宋皇室燒製。全世界已知的真品不到100件，台北故宮博物院收藏21件，北京故宮博物院收藏17件。有句俗話說："縱有家財萬貫，不抵汝瓷一件"。',
    },
    {
        id: 2,
        lotNumber: '002',
        title: 'AN IMPERIAL RU KILN "SKY BLUE" BOTTLE',
        chinese: '北宋 汝窯天青釉瓶',
        dynasty: 'song',
        dynastyInfo: 'Northern Song Dynasty, 1086-1106',
        dynastyInfoChinese: '北宋 (1086-1106年)',
        imageOrientation: 'portrait',
        images: [
            '/assets/images/relics/002-song-ru-bottle/main.jpg',
            '/assets/images/relics/002-song-ru-bottle/detail-1.jpg',
            '/assets/images/relics/002-song-ru-bottle/detail-2.jpg'
        ],
        thumbnail: '/assets/images/relics/002-song-ru-bottle/main.jpg',
        description: 'This magnificent bottle exemplifies the restraint and elegance that characterizes the finest Ru ware. Legend says Emperor Huizong once dreamt of a mysterious sky blue hue after rain, penning the verse "After the rain, the sky clears, and this color shall be created" (雨过天青云破处，这般颜色做将来). The bottle features the distinctive "crab claw" crackle pattern (蟹爪纹) unique to Ru kiln pieces, with each irregular crackle being unique.',
        descriptionChinese: '這件精美的瓶子體現了最優秀汝窯瓷器所具有的克制與優雅。傳說宋徽宗曾夢見雨後天青的神秘藍色，並寫下詩句"雨過天青雲破處，這般顏色做將來"。瓶子具有汝窯瓷器獨有的"蟹爪紋"開片圖案，每條不規則的開片都是獨一無二的。',
        estimate: { low: 65000000, high: 70000000 },
        currentBid: 65000000,
        provenance: 'Private Family Collection\nInherited through multiple generations\nDocumented family ownership history',
        provenanceChinese: '私人家族收藏\n世代傳承\n有記錄的家族所有權歷史',
        condition: 'Perfect condition. The glaze is of exceptional quality with the desirable "ice crackle" pattern throughout. Features the distinctive "fishtail" bubbles (鱼鳞纹) that cannot be replicated by modern methods.',
        conditionChinese: '保存完美。釉面品質極佳，遍布令人滿意的"冰裂紋"圖案。具有獨特的"魚鱗紋"氣泡，無法通過現代方法複製。',
        dimensions: 'Height: 22.8 cm\nDiameter: 12.5 cm\nWeight: 680g',
        dimensionsChinese: '高：22.8厘米\n直徑：12.5厘米\n重量：680克',
        literature: 'Referenced in scholarly works on Northern Song Ru ware ceramics\nComparable examples documented in major museum collections\nRu ware pieces have achieved record-breaking prices at auction',
        literatureChinese: '在關於北宋汝窯陶瓷的學術著作中被引用\n在主要博物館收藏中有可比較的實例記錄\n汝窯瓷器曾在拍賣中創下紀錄價格',
        technicalNotes: 'Planned verification through Shenbury\'s future AI Agent Swarm technology using high-resolution imaging and spectral analysis. The multi-agent system will cross-reference glaze composition, crackle patterns, and form characteristics against the AgentDB knowledge base. Each agent will specialize in different authentication aspects, with consensus validation ensuring accuracy. Development of this advanced verification infrastructure is supported by the Shenbury DAO.',
        technicalNotesChinese: '計劃通過神堡未來的AI代理群技術進行驗證，使用高分辨率成像和光譜分析。多代理系統將釉料成分、開片圖案和形制特徵與AgentDB知識庫進行交叉比對。每個代理將專注於不同的鑑定方面，通過共識驗證確保準確性。這一先進驗證基礎設施的開發由神堡DAO支持。',
        culturalSignificance: 'This bottle represents the golden age of Chinese ceramics during Emperor Huizong\'s reign (1082-1135). An outstanding painter and calligrapher, Huizong significantly advanced court arts and elevated Ru kiln\'s status. Ru ware has achieved exceptional prices at major auction houses.',
        culturalSignificanceChinese: '這件瓶子代表了宋徽宗統治期間（1082-1135年）中國陶瓷的黃金時代。作為傑出的畫家和書法家，徽宗大大推進了宮廷藝術並提升了汝窯的地位。汝窯瓷器曾在主要拍賣行創下 exceptional 價格。',
    },
    {
        id: 3,
        lotNumber: '003',
        title: 'A MAGNIFICENT MING XUANDE CLOISONNÉ TRIPOD CENSER',
        chinese: '明宣德 掐絲琺瑯獸鈕蓮紋三足爐',
        dynasty: 'ming',
        dynastyInfo: 'Xuande Period (1426-1435)',
        dynastyInfoChinese: '宣德年間 (1426-1435年)',
        imageOrientation: 'square',
        images: [
            '/assets/images/relics/003-ming-cloisonne/main.jpg',
            '/assets/images/relics/003-ming-cloisonne/detail-1.jpg',
            '/assets/images/relics/003-ming-cloisonne/detail-2.jpg',
        ],
        thumbnail: '/assets/images/relics/003-ming-cloisonne/main.jpg',
        description: 'This exceptional censer represents the golden age of Chinese cloisonné production during the Xuande period. Expertly crafted from high-quality copper, it features a flared rim, rounded body, and elegantly arched symmetrical handles. The lid is adorned with three evenly spaced gilt beast knobs, exuding regal presence. The body is covered with sky-blue enamel glaze, with red, yellow, blue, and other vibrant colors meticulously inlaid to create interwoven lotus scrolls.',
        descriptionChinese: '這件精美的香爐代表了宣德年間中國景泰藍製作的黃金時代。由高品質銅材精心製作，具有外撇口沿、圓潤器身和優雅拱形對稱把手。爐蓋裝飾有三個均勻分布的鍍金獸鈕，散發著皇家氣派。器身覆蓋天青色琺瑯釉，紅、黃、藍等鮮豔色彩精心鑲嵌，形成交織的蓮花卷草紋。',
        estimate: { low: 20000000, high: 25000000 },
        currentBid: 20000000,
        provenance: 'Private Family Collection\nLong-term family ownership\nDocumented collection history',
        provenanceChinese: '私人家族收藏\n長期家族擁有\n有記錄的收藏歷史',
        condition: 'Excellent condition. Minor restoration to one cloud-scroll on the rim professionally executed. The vibrant enamel colors remain rich and full.',
        conditionChinese: '保存完好。口沿上的一處雲紋卷草有專業修復。鮮豔的琺瑯色彩依然豐富飽滿。',
        dimensions: 'Height: 28.5 cm\nWidth: 35.2 cm\nWeight: 4.2 kg',
        dimensionsChinese: '高：28.5厘米\n寬：35.2厘米\n重量：4.2公斤',
        literature: 'Discussed in literature on Ming Dynasty cloisonné techniques\nRelated examples featured in museum collections and scholarly publications\nXuande period cloisonné pieces are highly valued by collectors',
        literatureChinese: '在關於明代景泰藍技術的文獻中被討論\n相關實例在博物館收藏和學術出版物中被介紹\n宣德年間的景泰藍器物受到收藏家高度重視',
        technicalNotes: 'Future authentication planned through Shenbury\'s specialized AI agents for metalwork and enamel analysis. The swarm will employ computer vision to analyze wire precision, color gradation, and gilding techniques. Machine learning models will be trained on verified Xuande period pieces to confirm authenticity markers, with ReasoningBank cross-validating findings against historical production techniques. This technology development is funded by the Shenbury DAO.',
        technicalNotesChinese: '計劃通過神堡專門的金屬工藝和琺瑯分析AI代理進行未來鑑定。代理群將使用計算機視覺分析金絲精度、色彩漸變和鍍金技術。機器學習模型將基於已驗證的宣德年間器物進行訓練以確認真實性標記，ReasoningBank將對比歷史生產技術交叉驗證發現。這一技術開發由神堡DAO資助。',
        culturalSignificance: 'Xuande marked cloisonné pieces are among the rarest and most sought-after works of Chinese decorative arts. This example bears the imperial six-character mark. The lotus scrolls symbolize continuous good fortune and prosperity. Xuande period pieces are highly prized by collectors.',
        culturalSignificanceChinese: '帶有宣德款的景泰藍器物是中國裝飾藝術中最稀有和最受追捧的作品之一。此例帶有皇家六字款。蓮花卷草象徵著連綿不斷的好運和繁榮。宣德年間的器物受到收藏家高度重視。',
    },
    {
        id: 4,
        lotNumber: '004',
        title: 'AN EXTRAORDINARILY RARE YUAN JILAN-GLAZED EWER',
        chinese: '元 霽藍釉白龍紋梨形執壺',
        dynasty: 'yuan',
        dynastyInfo: 'Yuan Dynasty (1271-1368)',
        dynastyInfoChinese: '元代 (1271-1368年)',
        imageOrientation: 'portrait',
        images: [
            '/assets/images/relics/004-yuan-jilan-ewer/main.jpg',
            '/assets/images/relics/004-yuan-jilan-ewer/detail-1.jpg',
            '/assets/images/relics/004-yuan-jilan-ewer/detail-2.jpg',
            '/assets/images/relics/004-yuan-jilan-ewer/detail-3.jpg'
        ],
        thumbnail: '/assets/images/relics/004-yuan-jilan-ewer/main.jpg',
        description: 'This spectacular ewer showcases the technical mastery achieved by Yuan potters in high-fired cobalt blue glazes. Most Yuan Dynasty porcelains are known for their large size, making this small ewer particularly rare. The Jilan glaze is considered the top variety of porcelain from the Yuan Dynasty, even more precious than the famed Yuan blue and white. The white dragon, depicted with its head raised and tongue extended, appears to be galloping through the sea.',
        descriptionChinese: '這件精美的執壺展示了元代陶工在高溫鈷藍釉方面的技術精湛。大多數元代瓷器以大尺寸著稱，使得這件小執壺格外稀有。霽藍釉被認為是元代瓷器的頂級品種，甚至比著名的元青花更珍貴。白龍抬頭吐舌，彷彿在海中奔騰。',
        estimate: { low: 17000000, high: 21000000 },
        currentBid: 17000000,
        provenance: 'Private Family Collection\nMulti-generational ownership\nDocumented collection records',
        provenanceChinese: '私人家族收藏\n多代擁有\n有記錄的收藏記錄',
        condition: 'Remarkable condition. Small chip to the spout professionally restored. The cobalt blue glaze of exceptional depth and brilliance provides a striking backdrop for the elegant white dragon.',
        conditionChinese: '保存狀況出色。壺嘴的小缺口已專業修復。深邃明亮的鈷藍釉為優雅的白龍提供了引人注目的背景。',
        dimensions: 'Total Height (with lid): 31.3 cm\nWidth (handle to spout): 18.5 cm\nWeight: 1.45 kg',
        dimensionsChinese: '總高（含蓋）：31.3厘米\n寬度（把手到壺嘴）：18.5厘米\n重量：1.45公斤',
        literature: 'Featured in scholarly discussions of Yuan dynasty ceramic innovations\nComparable pieces documented in museum collections\nYuan dynasty blue-glazed ceramics are highly prized by collectors',
        literatureChinese: '在關於元代陶瓷創新的學術討論中被介紹\n在博物館收藏中有可比較的器物記錄\n元代藍釉陶瓷受到收藏家高度重視',
        technicalNotes: 'Intended verification using Shenbury\'s ceramic authentication swarm with specialized agents for Yuan dynasty glazes. The AI system will analyze cobalt distribution patterns, glaze thickness, and dragon motif execution. Local vector embeddings will compare the piece against known authentic Yuan ceramics in the future AgentDB, with closed-loop learning continuously improving verification accuracy. This technology development is funded by the Shenbury DAO.',
        technicalNotesChinese: '計劃使用神堡陶瓷鑑定代理群進行驗證，該群體具有專門的元代釉料代理。AI系統將分析鈷分佈模式、釉層厚度和龍紋執行。本地向量嵌入將把此器物與未來AgentDB中已知的真品元代陶瓷進行比較，閉環學習不斷提高驗證準確性。這一技術開發由神堡DAO資助。',
        culturalSignificance: 'Yuan dynasty blue glazed porcelain represents a revolutionary moment in ceramic history, combining Chinese potting techniques with Islamic aesthetic preferences. The dragon motif adds movement and vitality, embodying imperial power. Yuan blue-glazed pieces are highly sought after by collectors.',
        culturalSignificanceChinese: '元代藍釉瓷器代表了陶瓷史上的革命性時刻，結合了中國製陶技術和伊斯蘭美學偏好。龍紋增添了動感和活力，體現了皇權。元代藍釉器物受到收藏家高度重視。',
    },
    {
        id: 5,
        lotNumber: '005',
        title: 'IMPERIAL JINGTAI CLOISONNÉ LOTUS PATTERN LIDDED CASKET',
        chinese: '明景泰 掐絲琺瑯蓮紋蓋盒',
        dynasty: 'ming',
        dynastyInfo: 'Jingtai Period (1450-1457)',
        dynastyInfoChinese: '景泰年間 (1450-1457年)',
        imageOrientation: 'square',
        images: [
            '/assets/images/relics/005-ming-jingtai-casket/main.jpg',
            '/assets/images/relics/005-ming-jingtai-casket/detail-1.jpg',
            '/assets/images/relics/005-ming-jingtai-casket/detail-2.jpg',
        ],
        thumbnail: '/assets/images/relics/005-ming-jingtai-casket/main.jpg',
        description: 'This masterpiece of Chinese decorative arts epitomizes the pinnacle of cloisonné craftsmanship from the Jingtai period. The casket features a perfectly round shape with a fitted lid and circular base, embodying symmetry and balance. The body is adorned with gilded lotus petal patterns, with blue enamel glaze serving as the background, filled with various colors of enamel to create intricate, entwined lotus patterns.',
        descriptionChinese: '這件中國裝飾藝術的傑作體現了景泰年間景泰藍工藝的巔峰。蓋盒呈完美圓形，配有合適的蓋子和圓形底座，體現了對稱與平衡。器身裝飾有鍍金蓮花瓣圖案，以藍色琺瑯釉為背景，填入各種顏色的琺瑯，形成複雜交織的蓮花圖案。',
        estimate: { low: 15000000, high: 16000000 },
        currentBid: 15000000,
        provenance: 'Private Family Collection\nLong-term family ownership\nDocumented ownership history',
        provenanceChinese: '私人家族收藏\n長期家族擁有\n有記錄的所有權歷史',
        condition: 'Excellent condition with bright, full glaze. The patina that has developed over time adds to its allure, giving it an ancient yet elegant appearance. All original gilding intact.',
        conditionChinese: '保存完好，釉面明亮飽滿。隨著時間發展的包漿增加了其魅力，使其具有古樸而優雅的外觀。所有原始鍍金完好無損。',
        dimensions: 'Diameter: 22.5 cm\nHeight: 12.8 cm\nWeight: 2.1 kg',
        dimensionsChinese: '直徑：22.5厘米\n高：12.8厘米\n重量：2.1公斤',
        literature: 'Referenced in works on Jingtai period cloisonné craftsmanship\nRelated examples in major decorative arts collections\nJingtai period cloisonné pieces are highly valued by collectors',
        literatureChinese: '在關於景泰年間景泰藍工藝的著作中被引用\n在主要裝飾藝術收藏中有相關實例\n景泰年間的景泰藍器物受到收藏家高度重視',
        technicalNotes: 'Planned authentication through Shenbury\'s multi-agent verification system with specialized cloisonné experts. The swarm will analyze wirework precision, enamel color consistency, and gilding quality using computer vision. Pattern recognition algorithms will compare decorative motifs against verified Jingtai period examples, with confidence scoring provided by ReasoningBank\'s closed-loop learning system. This advanced technology is under development and funded by the Shenbury DAO.',
        technicalNotesChinese: '計劃通過神堡多代理驗證系統進行鑑定，該系統具有專門的景泰藍專家。代理群將使用計算機視覺分析金絲工藝精度、琺瑯色彩一致性和鍍金品質。模式識別算法將裝飾紋樣與已驗證的景泰年間實例進行比較，信心評分由ReasoningBank的閉環學習系統提供。這項先進技術正在開發中，由神堡DAO資助。',
        culturalSignificance: 'The Jingtai period is renowned for its advancements in cloisonné enamel techniques. This casket exemplifies the luxurious and noble characteristics unique to cloisonné enamel, demonstrating the dedication to creating objects of both beauty and function. Jingtai period pieces are considered among the finest examples of Chinese decorative arts.',
        culturalSignificanceChinese: '景泰年間以其景泰藍琺瑯技術的進步而聞名。這件蓋盒體現了景泰藍琺瑯獨有的奢華和高貴特徵，展示了創造美觀實用器物的 dedication。景泰年間的器物被收藏家高度重視。',
    },
    {
        id: 6,
        lotNumber: '006',
        title: 'NORTHERN SONG DYNASTY RUGUAN KILN YUHU CHUN VASE',
        chinese: '北宋 汝官窯玉壺春瓶',
        dynasty: 'song',
        dynastyInfo: 'Northern Song Dynasty, 1086-1106',
        dynastyInfoChinese: '北宋 (1086-1106年)',
        imageOrientation: 'portrait',
        images: [
            '/assets/images/relics/006-song-yuhuchun-vase/main.jpg',
            '/assets/images/relics/006-song-yuhuchun-vase/detail-1.jpg',
            '/assets/images/relics/006-song-yuhuchun-vase/detail-2.jpg',
            '/assets/images/relics/006-song-yuhuchun-vase/detail-3.jpg',
            '/assets/images/relics/006-song-yuhuchun-vase/detail-4.jpg'
        ],
        thumbnail: '/assets/images/relics/006-song-yuhuchun-vase/main.jpg',
        description: 'This exceptionally rare Yuhuchun vase from the Northern Song Dynasty Ruguan Kiln stands at 22 centimeters tall, making it one of only two known examples in the world. The vase features the characteristic sky-blue glaze of Ru ware with its distinctive crackle pattern. Its elegant form, with a gently curved body that tapers toward the base, exemplifies the refined aesthetic of Song dynasty ceramics. The glaze exhibits the pure azure color that made Ru ware the most prized of all Chinese ceramics.',
        descriptionChinese: '這件來自北宋汝官窯的極其稀有的玉壺春瓶高22厘米，是世界上僅知的兩件實例之一。花瓶具有汝窯特徵性的天青釉和獨特的開片圖案。其優雅的造型，器身優雅地向底部收攏，體現了宋代陶瓷的精緻美學。釉面呈現使汝窯成為所有中國陶瓷中最受推崇的純凈天青色。',
        estimate: { low: 100000000, high: 120000000 },
        currentBid: 100000000,
        provenance: 'Private Family Collection\nMulti-generational family ownership\nDocumented collection history',
        provenanceChinese: '私人家族收藏\n多代家族擁有\n有記錄的收藏歷史',
        condition: 'Excellent condition with the characteristic ice-crackle glaze throughout. The piece has been professionally examined and authenticated by experts in Song dynasty ceramics. The foot ring shows minor expected wear consistent with age and handling.',
        conditionChinese: '保存完好，遍布特徵性的冰裂紋釉。此器物已由宋代陶瓷專家專業檢查和鑑定。足圈有與年代和使用相符的輕微磨損。',
        dimensions: 'Height: 22 cm\nMouth diameter: 5.5 cm\nBase diameter: 7.2 cm',
        dimensionsChinese: '高：22厘米\n口徑：5.5厘米\n底徑：7.2厘米',
        literature: 'Documented in scholarly works on Song dynasty ceramics\nComparable examples referenced in museum collections and academic publications\nRu ware pieces have achieved record-breaking prices at auction',
        literatureChinese: '在關於宋代陶瓷的學術著作中有記錄\n在博物館收藏和學術出版物中引用了可比較的實例\n汝窯瓷器曾在拍賣中創下紀錄價格',
        technicalNotes: 'Future verification by Shenbury\'s advanced AI authentication swarm with Ru ware specialists. The system will employ high-resolution imaging to analyze glaze crackle patterns, foot ring construction, and form proportions. Machine learning models will compare the piece against verified Song dynasty ceramics in the future AgentDB, with confidence scores generated through ReasoningBank\'s continuous learning framework. This cutting-edge technology is currently under development funded by the Shenbury DAO.',
        technicalNotesChinese: '未來由神堡先進的AI鑑定代理群進行驗證，該群體具有汝窯專家。系統將使用高分辨率成像分析釉面開片圖案、足圈構造和造型比例。機器學習模型將把此器物與未來AgentDB中已驗證的宋代陶瓷進行比較，信心評分通過ReasoningBank的持續學習框架生成。這項尖端技術目前正在由神堡DAO資助開發中。',
        culturalSignificance: 'The Yuhuchun vase form is one of the most celebrated shapes in Chinese ceramic history. With only two known examples in existence, this piece represents an extraordinary rarity. The taller stature compared to the British Museum example makes it particularly significant. Ru ware pieces were exclusively produced for the Northern Song imperial court for approximately 20 years (1086-1106), contributing to their extreme scarcity. Ru ware is among the most valuable ceramics in the world.',
        culturalSignificanceChinese: '玉壺春瓶造型是中國陶瓷史上最受讚譽的造型之一。由於僅存兩件已知實例，此器物代表了非凡的稀有性。與大英博物館藏品相比更高的身量使其特別重要。汝窯瓷器僅為北宋皇室專門生產約20年（1086-1106年），這導致了它們的極端稀少。汝窯瓷器是世界上最珍貴的陶瓷之一。'
    }
];